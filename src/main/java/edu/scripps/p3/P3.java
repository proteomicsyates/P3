/**
 * Protein Protein interaction Predictor (P3) is a framework to deconvolve complex dataset and to 
 * build a full interaction network
 *  
 * @author diego
 * Jun 11, 2013
 */
...
/**
 * Main class of P3, it invokes the GUI and then passes the collected values to the P3Core 
 * @author diego
 * @version 1.0
 */
public class P3 {
	...
	/**
	 * invokes the P3mainFrame method to create the gui
	 * creates a separate thread to deal with it
	 * collects the results
	 * invokes the P3Core method
	 * shows time used by the core
	 */
	public void run() {
		
		ppp3gui = new P3mainFrame(elist, qlist, llist, olist,
				quantitative, lysate, physical, genetic, bonus, indirect,
				advanced, outdir, logdir, topodir, baits, experiments,
				configuration, lock);
		...
		try {
			...
			p3core = new P3Core(elist, qlist, llist, olist,
						quantitative, lysate, physical, genetic, bonus, indirect,
						advanced, outdir, logdir, topodir, baits, experiments,
						configuration);
			...
			JOptionPane.showMessageDialog(null,"Execution Terminated in " + time);
