...
public class ConfidenceStratifier {
	 ...
	 * @param logdir
	 */
	public ConfidenceStratifier(List<List<Interactome>> interactomes,
			File logdir) {
		this.logdir = logdir;
	...
	private void stratify() {
		...
		for (int i = 0; i < interactomes.size(); i++) {
			for (int j = 0; j < interactomes.get(i).size(); j++) {
				...
				w_size = interactome.getNetlist().size();
				...
				for (int k = 0; k < w_size; k++) {
					...
					Network network = interactome.getNetwork(interactome
							.getNetlist().get(k));

					List<String> interactors = network.getInteraction_names();
					...
					for (String interactor : interactors) {
						...
						if (values.containsKey(interactor)) {
							...
							scores[k] = network.getInteraction(interactor)
									.getConfidence_score();
				...
				//		}
						...
						if (scores[k] > 0) {
							interactome
									.getNetwork(interactome.getNetlist().get(k))
									.getInteraction(key)
	...
	private void filter() {
		...
		for (int k = 0; k < interactomes.size(); k++) {
			...
			for (int i = 0; i < interactomes.get(k).size(); i++) {
				...
				log.append("Working on\t"
						+ interactomes.get(k).get(i).getExp_name() + "\n");

				expbaits = interactomes.get(k).get(i).getBait_name().split("-");
				...
				for (int b = 0; b < expbaits.length; b++) {
					...
					Network bait = interactomes.get(k).get(i)
							.getNetwork(current_bait);
					...
					for (String interactor : interactors) {
						...
						row.append(i_score + "\t" + inter.getCorrelation_score() + "\t" + inter.getCluster_score() + "\t" + inter.getQuant_score() + "\t" + p_score + "\t" + g_score + "\t" + score);
						if (lys==1) {
							row.append("Y\t");
						} else {
							row.append("\t");
						...
						if (score > 0.1) {
							...
							map.put(interactor, score + "_" + i_score + "_"
									+ p_score + "_" + g_score + "_" + lys + "_"
									+ qbeh);
