...
public class FilterRealExperiment {
	...
	protected void filter() throws IOException {
		...
		BufferedReader in = new BufferedReader(new FileReader(controlExperimentFile));

		significativeInteractorsFile = FileUtils.appendToFileName(controlExperimentFile, null, "_Sig");
		BufferedWriter outPassed = new BufferedWriter(new FileWriter(significativeInteractorsFile));
		noSignificativeInteractorsFile = FileUtils.appendToFileName(controlExperimentFile, null, "_Non_Sig");
		BufferedWriter outFilteredOut = new BufferedWriter(new FileWriter(noSignificativeInteractorsFile));
		notEnoughReplicatesFile = FileUtils.appendToFileName(controlExperimentFile, null, "_Not_Enough_Replicates");
		BufferedWriter outNotEnoughRep = new BufferedWriter(new FileWriter(notEnoughReplicatesFile));

		outPassed.write(
				"UniProt_Accession\tt-test_p-value\tPassed_With_Singleton_Criteria\tHeavy_Light\tAverage_Ratio\tProtein_Info\tRatios\n");
		...
		while (s != null) {
			...
			String UniProt_Acc = split[indexesByHeaders.get(PreFilterUtils.ACC)];
			String ProteinInfo = split[indexesByHeaders.get(PreFilterUtils.DESCRIPTION)];
