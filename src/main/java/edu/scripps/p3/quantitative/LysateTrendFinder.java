				...
				if (lvalues.containsKey(key)) {
					...
					if (ratio>lowT && ratio < highT) {
						...
						for (int j=0; j < interactomes.get(i).size(); j++) {
							...
							for (String network : interactome.getNetlist()) {
								
								if (interactome.getNetwork(network).isInNetwork(key)) {
									
									interactome.getNetwork(network).getInteraction(key).setLysateLike(true);
