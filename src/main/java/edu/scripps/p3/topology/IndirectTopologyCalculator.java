...
public class IndirectTopologyCalculator {
	 ...
	 * @param maps_names
	 */
	public IndirectTopologyCalculator(List<List<Interactome>> interactomes,
			String[] baits, List<List<Hashtable<String, String>>> maps,
			List<List<String>> maps_names) {
		...
		for (int k=0; k < interactomes.size(); k++) {
			...
			for (Interactome interactome : interactomes.get(k)) {
				...
				for (String bait : expbaits) {
					...
					key = bait + "_" + interactome.getExp_name();
					...
					if (index != -1) {
						...
						for (String pname : pnames) {
							...
							if (interactome.isNetworkinSystem(pname)) {
								...
								root = net.getExpName();
								interactions = net.getInteraction_names();
								
								for (String interaction : interactions) {
									
									if (pnames.contains(interaction)) {
										
										Interaction inter = net
												.getInteraction(interaction);

										
										c_score = inter.getCorrelation_score();
										if (c_score>0) {
											indirect.append(root + "\t"
													+ interaction + "\ti "
													+ 9 + "\tc "
													+ (c_score*0.2) + "\n");
										}
