...
public class TopologyCreator {
	...
	private final List<List<Hashtable<String, String>>> maps;
	private final List<List<String>> maps_names;
	 ...
	 * @param qlist
	 */
	public TopologyCreator(List<List<Hashtable<String, String>>> maps, List<List<String>> maps_names, File topodir,
	...
	public void run() {
		...
		for (int k = 0; k < maps_names.size(); k++) {
			...
			for (int i = 0; i < pairs.size(); i++) {
				...
				dmc = new DifferentialMapCreator(maps_names.get(k).get(index1), maps.get(k).get(index1),
						maps.get(k).get(index2), qlist.get(k));
				if (indirect) {
